...
/**
 * Central event dispatcher for DecompAI task communication.
...
public class EventDispatcher {
    ...
    private final ConcurrentHashMap<DecompaiEvent.EventType, List<EventConsumer>> subscriptions;
    
    // Track all consumers for unsubscribe operations
    private final ConcurrentHashMap<EventConsumer, Set<DecompaiEvent.EventType>> consumerSubscriptions;
    ...
    public void subscribe(EventConsumer consumer) {
        ...
        Set<DecompaiEvent.EventType> eventTypes = consumer.getSubscribedEventTypes();
        ...
        for (DecompaiEvent.EventType eventType : eventTypes) {
    ...
    public void unsubscribe(EventConsumer consumer) {
        ...
        Set<DecompaiEvent.EventType> eventTypes = consumerSubscriptions.remove(consumer);
        if (eventTypes == null) {
     ...
     */
    public void publish(DecompaiEvent event) {
        if (event == null) {
            return;
        }
     ...
     */
    public int getSubscriberCount(DecompaiEvent.EventType eventType) {
        List<EventConsumer> consumers = subscriptions.get(eventType);
